...
test.describe('Store Module Tests', () => {
    ...
    test('SM-011: Verify recent phone call', async () => {
        const text = await getCellText(0, 10);
        expect(text).toBeTruthy();
    });
    ...
    test('SM-015: Verify name sorting', async () => {
        ...
        const firstValAsc = await getCellText(0, 1);

        await header.scrollIntoViewIfNeeded();
        await header.click();
        await page.getByRole('menuitem', { name: /Desc/i }).click();
        await page.waitForTimeout(5000);
        const firstValDesc = await getCellText(0, 1);
    ...
    test('SM-016: Verify pagination', async () => {
        await page.waitForLoadState('networkidle');
        const firstPageData = await getCellText(0, 1);

        const nextBtn = page.locator('button:has(svg.lucide-chevron-right)').last();
        await nextBtn.scrollIntoViewIfNeeded();
        await nextBtn.click();
        await page.waitForTimeout(8000);
        const secondPageData = await getCellText(0, 1);
        expect(firstPageData).not.toBe(secondPageData);
    });

    // test('SM-017: Verify store search', async () => {
    //     // Reload page to ensure clean state after pagination
    //     await page.goto('https://app.omnigrowthos.io/stores');
    //     await page.waitForLoadState('networkidle');
    //     await page.waitForTimeout(2000);
        
    //     // Precise name extraction from first row
    //     const firstName = await page.locator('tbody tr').first().locator('td').nth(1).locator('span.font-bold').textContent();
    //     const cleanName = firstName ? firstName.trim() : '';
    //     console.log(`Searching for: [${cleanName}]`);

    //     // Perform Search - use placeholder locator which is more reliable
    ...
    //     const resultText = await getCellText(0, 1);
    ...
    test('SM-020: Verify data consistency', async () => {
        const name = await getCellText(0, 1);
        const ranking = await getCellText(0, 2);
