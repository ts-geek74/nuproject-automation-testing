class PhoneCallPage {
     ...
     * Note: Comments and Photos tabs may not exist for all phone calls
     */
    async testDetailsTabs() {
        try {
            const results = {
                commentsTab: { exists: false, success: false, hasContent: false },
                photosTab: { exists: false, success: false, hasContent: false }
            };

            // Test Comments tab (may not exist)
            ...
            if (commentsTabExists) {
                ...
                const hasComments = !(await this.noCommentsMessage.isVisible({ timeout: 2000 }).catch(() => false));
                const commentsContainer = this.page.locator('[data-comments], .comments-list').first();
                const hasCommentsContainer = await commentsContainer.isVisible({ timeout: 2000 }).catch(() => false);

                results.commentsTab = {
                    exists: true,
                    success: true,
                    hasContent: hasComments || hasCommentsContainer,
                    isEmpty: !hasComments && !hasCommentsContainer
                };
            } else {
                console.log('Comments tab does not exist for this phone call');
            }

            // Test Photos tab (may not exist)
            ...
            if (photosTabExists) {
                ...
                const photosContainer = this.page.locator('[data-photos], .photos-grid, .photo-gallery').first();
                const noPhotosMsg = this.page.getByText(/no photos/i).first();

                const hasPhotos = await photosContainer.isVisible({ timeout: 2000 }).catch(() => false);
                const showsNoPhotos = await noPhotosMsg.isVisible({ timeout: 2000 }).catch(() => false);

                results.photosTab = {
                    exists: true,
                    success: true,
                    hasContent: hasPhotos,
                    isEmpty: showsNoPhotos || !hasPhotos
                };
            } else {
                console.log('Photos tab does not exist for this phone call');
            }
